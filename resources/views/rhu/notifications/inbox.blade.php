...
<div class="container-fluid px-4">
    ...
    @php
        ...
        $thisWeek     = collect($sent)->filter(fn($n) => isset($n['createdAt']) && \Carbon\Carbon::parse($n['createdAt'])->isCurrentWeek())->count();
        $healthAlerts = collect($sent)->where('notification_type', 'health_alert')->count();
        $inboxUnread  = collect($inbox)->filter(fn($n) => ($n['status'] ?? '') === 'unread')->count();
    @endphp
    ...
    {{-- ── SENT TAB ── --}}
    <div id="paneSent" class="notif-list-card" style="border-top-left-radius:0;">
        <div class="notif-list-header d-flex align-items-center justify-content-between px-4 py-3">
            ...
            <div class="notif-search-wrap">
                ...
                <input type="text" class="notif-search-input" id="notifSearch" placeholder="Search…" oninput="filterNotifications(this.value)">
            </div>
        </div>
            ...
            <table class="table notif-table mb-0" id="notifTable">
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $targetStr = is_array($targets)
                            ? implode(', ', array_slice($targets, 0, 2)) . (count($targets) > 2 ? ' +' . (count($targets) - 2) . ' more' : '')
                            : ($targets ?: '—');
                        $audienceMap = [
                    ...
                    <tr class="notif-row"
                        ...
                        <td><span class="notif-targets">{{ $targetStr }}</span></td>
        ...
        <div class="notif-empty">
            <i class="bi bi-send notif-empty-icon"></i>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="wizard-pane" id="wizardStep1">
                        <div class="modal-form-section mb-3">
                                ...
                                <div class="bgy-panel" id="bgyPanel">
                                    ...
                                    <div class="bgy-select-all-row">
                                        <label class="bgy-option bgy-select-all-label">
                                            ...
                                            <span class="bgy-count-badge">{{ count($barangays) }}</span>
                                        </label>
                                    </div>
                            ...
                            @else
                            <div class="empty-bgy-notice"><i class="bi bi-info-circle me-2"></i>No barangays found under this RHU.</div>
                            @endif
                        </div>
        ...
        <div class="modal-content rounded-4 overflow-hidden border-0 shadow-lg">
            ...
            <div class="modal-body p-4">
                <div class="modal-form-section mb-3">
                    ...
                    <div class="review-row"><span class="review-label">Barangay(s)</span><span class="review-value" id="detailBarangays">—</span></div>
                    <div class="review-row"><span class="review-label">Audience</span><span class="review-value" id="detailAudience">—</span></div>
                    <div class="review-row"><span class="review-label">Date Sent</span><span class="review-value" id="detailDate">—</span></div>
                    <div class="review-row"><span class="review-label">Status</span><span id="detailStatus">—</span></div>
                </div>
                <div class="modal-form-section">
...
.notif-tab { background:#f8fafc;border:1px solid #e9e9e7;border-bottom:none;border-radius:8px 8px 0 0;padding:9px 20px;font-size:.82rem;font-weight:500;color:#787774;cursor:pointer;transition:all .15s;display:flex;align-items:center;gap:6px; }
.notif-tab:first-child { margin-right:4px; }
.notif-tab.active { background:#fff;color:#37352f;font-weight:600;border-color:#e9e9e7;position:relative;z-index:1; }
.tab-count { background:#f1f1ef;color:#787774;font-size:.65rem;font-weight:700;border-radius:10px;padding:1px 6px; }
.tab-count-unread { background:#fee2e2;color:#b91c1c;font-size:.65rem;font-weight:700;border-radius:10px;padding:1px 6px; }

.notif-list-card { border:1px solid #e9e9e7;border-radius:0 8px 8px 8px;background:#fff; }
...
.notif-search-input { padding:6px 12px 6px 30px;border:1px solid #e9e9e7;border-radius:6px;font-size:.82rem;color:#37352f;outline:none;width:220px; }
...
.wizard-steps { display:flex;align-items:center;justify-content:center; }
.wizard-step { display:flex;flex-direction:column;align-items:center;gap:4px; }
.wizard-step-line { height:2px;background:#e9e9e7;width:60px;margin:0 8px 16px; }
.step-circle { width:28px;height:28px;border-radius:50%;background:#e9e9e7;color:#9b9b9b;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:700;transition:all .2s; }
...
/* Review step */
.review-header { display:flex;align-items:center;gap:12px;padding:14px 16px;background:#fafaf9;border:1px solid #f1f1ef;border-radius:6px; }
...
function switchTab(tab) {
    document.getElementById('paneSent').classList.toggle('d-none', tab !== 'sent');
...
function openWizard() {
    if (!wizardModalInstance) wizardModalInstance = new bootstrap.Modal(document.getElementById('wizardModal'));
    resetWizard();
...
function resetWizard() {
    ...
    const sa = document.getElementById('selectAllBarangays');
...
function goNext() { if (currentStep < 3) { currentStep++; showStep(currentStep); } }
...
function validateStep1() {
    ...
    if (ok && audience?.value === 'specific_purok')     ok = !!document.getElementById('target_purok').value.trim();
    if (ok && audience?.value === 'specific_age_group') ok = !!document.getElementById('target_age_group').value.trim();
...
function populateReview() {
    ...
    const audience = document.querySelector('input[name="target_audience"]:checked');
    const audienceMap = { 'all_residents':'All Residents','specific_purok':'Specific Purok','specific_age_group':'Age Group','pregnant_women':'Pregnant Women','senior_citizens':'Senior Citizens','children_0_12':'Children (0–12)' };
    ...
    const typeLabels = { 'health_alert':'Health Alert','announcement':'Announcement','reminder':'Reminder','vaccination_update':'Vaccination','clinic_schedule_update':'Clinic Schedule' };
    const classByType = { 'health_alert':'nbadge-danger','announcement':'nbadge-primary','reminder':'nbadge-warning','vaccination_update':'nbadge-success','clinic_schedule_update':'nbadge-info' };
    document.getElementById('reviewType').innerHTML = typeVal ? `<span class="nbadge ${classByType[typeVal] || 'nbadge-primary'}">${typeLabels[typeVal] || typeVal}</span>` : '—';
...
document.addEventListener('click', function(e) {
    const dd = document.getElementById('bgyDropdown');
    if (dd && !dd.contains(e.target)) { bgyPanel.classList.remove('open'); bgyTrigger.classList.remove('open'); }
});
function filterBgy(q) {
    const lower = q.toLowerCase();
    document.querySelectorAll('#bgyOptionsList .bgy-option').forEach(opt => opt.classList.toggle('hidden', !opt.dataset.name.includes(lower)));
}
function toggleSelectAll(cb) { allBgyCheckboxes().forEach(c => { c.checked = cb.checked; }); updateBgyState(); }
...
function removeBgyTag(val) { const cb = document.querySelector(`.barangay-checkbox[value="${val}"]`); if (cb) { cb.checked = false; onBgyChange(); } }
...
function onMessageInput() {
    document.getElementById('charCount').textContent = document.getElementById('message').value.length + ' / 2000';
    validateStep2();
}
...
function filterNotifications(q) {
    const lower = q.toLowerCase();
    document.querySelectorAll('.notif-row').forEach(row => { row.style.display = row.dataset.search.includes(lower) ? '' : 'none'; });
}
